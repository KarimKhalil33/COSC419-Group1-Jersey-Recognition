...
class LossCallback(Callback):
    def __init__(self):
        ...
        # Ensure output directories exist before any save attempt
        self.loss_dir = Path("loss_data")
        self.images_dir = self.loss_dir / "images"
    ...
    def on_after_backward(self, trainer, pl_module):
        ...
        if pl_module.trainer.logged_metrics.get("loss", None) is not None:
            ...
            # Create a fresh figure each time and close it after saving
            plt.figure()
            ...
            plt.ylabel("Loss")
            plt.title(
                f"PARSeq Fine-Tuning Training Loss, 20 Batch Moving Average [Epoch {trainer.current_epoch}]"
            )
            plt.savefig(self.images_dir / f"loss_epoch{trainer.current_epoch}_{self.figure_num}.png")
            plt.close()
    ...
    def on_train_epoch_end(self, trainer, pl_module, dataloader_idx=None):
        ...
        file_path = self.loss_dir / f"losses_epoch_{epoch}.txt"
