print("[BOOT] main_fast.py starting", flush=True)
import argparse
...
def _sharpness_laplacian_var_bgr(img_bgr: "np.ndarray") -> float:
...
def select_topk_crops_per_tracklet(crops_imgs_dir: str, k: int, min_keep: int = 1, verbose: bool = True) -> None:
    """
    ...
    files = [f for f in os.listdir(crops_imgs_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.webp'))]
    if len(files) == 0:
        if verbose:
            print(f"[TopK] no image files found in {crops_imgs_dir}")
        return

    # Group by tracklet id (prefix before first underscore)
    by_track = {}
    for fn in files:
        track = fn.split('_')[0]
...
def clean_soccer_net_artifacts(part: str, clean_crops: bool = True, verbose: bool = True) -> None:
    """Remove (overwrite) outputs from previous runs so a rerun reflects full pipeline time."""
    try:
...
def soccer_net_pipeline(args):
    ...
    print("illegibility path and gt path")
    ...
    print("input and output json paths for pose estimation")
    ...
    if args.pipeline['crops'] and success:
        ...
        try:
            ...
            # Optional: keep only top-K sharpest crops per tracklet (speed + often accuracy)
    ...
    if args.pipeline['str'] and success:
        ...
        # Optional: Top-K pruning can be applied even if crops were generated earlier (i.e., crops stage is skipped).
        if args.topk_crops > 0:
            crops_imgs_dir = os.path.join(image_dir, 'imgs')
        ...
        command = f"conda run -n {config.str_env} python3 str.py  {config.dataset['SoccerNet']['str_model']}\
            --data_root={image_dir} --batch_size={args.str_batch_size} --inference --result_file {str_result_file}"
        success = os.system(command) == 0
        print("Done predict numbers")
    ...
    if args.pipeline['combine'] and success:
        ...
        # Bayesian combiner (more robust than simple voting).
        # We try TS/logits path first; fall back to raw if needed.
        try:
            ...
            try:
                results_dict, analysis_results = helpers.process_jersey_id_predictions_bayesian(str_result_file, useTS=False, useBias=True, useTh=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--full_pipeline', action='store_true', default=False,
                        help='Run the entire pipeline (feat+filter+legible+pose+crops+str+combine+eval). Default is only str+combine+eval.')
    parser.add_argument('--clean', action='store_true', default=False,
                        help='Delete previous run artifacts (and crops) before running, so outputs are overwritten and timing reflects a full fresh run.')
    parser.add_argument('--keep_crops', action='store_true', default=False,
                        help='When used with --clean, keep existing crops folder (do not delete crops).')
