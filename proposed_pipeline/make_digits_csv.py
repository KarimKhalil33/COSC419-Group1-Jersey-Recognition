...
OUT_DIR   = os.path.join(BASE_DIR, "splits_digits")

TRAIN_OUT = os.path.join(OUT_DIR, "train_digits.csv")
VAL_OUT   = os.path.join(OUT_DIR, "val_digits.csv")
...
def convert(in_csv: str, out_csv: str, bad_txt: str):
    with open(in_csv, "r", newline="") as f:
        ...
        for row in reader:
            ...
            # ------------------------------------
            # EXCLUDE -1 LABELS (KEY DIFFERENCE)
            # ------------------------------------
            if y == -1:
                continue
